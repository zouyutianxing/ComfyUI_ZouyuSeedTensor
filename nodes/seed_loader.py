...
6. 输出「张量输出(conditioning)」+「引导器(GUIDER)」+「Latent」+「日志(logs)」

- 参考端口使用官方 io.Autogrow.Input + TemplatePrefix(min=0, max=50) 动态扩展
- 视频端口直接接收 VIDEO（内部提取帧 + 音轨）
- 打包前可备份到永久/临时目录
- 输出前卸载 clip/vae/audio_vae 显存
- OOM 修复：参考 VAE 编码后 unload_all_models 卸载 VAE 再文本编码；帧数超训练范围告警/钳制
...
# MiniMax H3 训练帧数范围（官方注释 ~124-362 帧），超此范围会显著增加显存且效果不可控
...
class ZouyuSeedLoader(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Model.Input("model", tooltip="MiniMax H3 模型（用于构建引导器 GUIDER）"),
    ...
    def execute(cls, clip, vae, audio_vae, model, prompt, width, height, duration, fps,
        ...
        # ---- 5. 重新解码编码（内存管理：VAE 编码后卸载 VAE 再文本编码）----
        ...
        if has_ref:
            ...
            # OOM 修复：卸载 VAE，为文本编码腾出显存
            logz("卸载 VAE 以释放显存，准备文本编码…")
            free_memory()
            try:
        ...
        logz(f"已打包张量文件 -> {temp_path} ({os.path.getsize(temp_path) / (1024 * 1024):.1f} MB)")
