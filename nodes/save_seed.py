"""
节点: ZouyuSaveSeedConditioning（保存种子张量）— V3 API

将 conditioning 张量 + 种子 + 参考媒体（图片/视频/音频）+ 溯源元数据打包保存。
支持保存到永久目录（seeds/）或临时目录（temp/）。

参考端口使用官方 io.Autogrow.Input + TemplatePrefix 动态扩展（参考图最多 50 个）。
"""
...
class ZouyuSaveSeedConditioning(io.ComfyNode):
    """保存 conditioning + 种子 + 参考媒体 + 溯源元数据。"""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="ZouyuSaveSeedConditioning",
            display_name="保存种子张量 (Zouyu Save)",
            category="ZouyuAI/SeedTensor",
            is_output_node=True,
            description="将 conditioning + 种子 + 参考媒体打包保存到永久/临时目录。",
            inputs=[
                io.Conditioning.Input("conditioning",
                                      tooltip="来自 MiniMaxH3ReferenceToVideo 或融合节点的 conditioning 输出"),
                io.Int.Input("seed", default=0, min=0, max=0xFFFFFFFFFFFFFFFF, tooltip="当前使用的随机种子"),
                io.String.Input("filename", default="my_seed", tooltip="保存文件名（不含扩展名），如 shot_001_black_cat"),
                io.Combo.Input("storage", options=["permanent", "temp"], default="permanent",
                               tooltip="存储位置：permanent=seeds/ 永久目录；temp=temp/ 临时目录"),
                io.Combo.Input("language", options=["中文", "English"], default="中文"),
                io.Combo.Input("canvas_mode", options=["auto", "max", "custom"], default="auto",
                               tooltip="画布计算模式：auto=按参考图宽高比自适应；max=使用给定/默认尺寸；custom=使用宽度/高度"),
                io.Int.Input("width", default=0, min=0, max=8192, step=32, tooltip="目标宽度（0=自动）。MiniMax H3 要求 32 的倍数"),
                io.Int.Input("height", default=0, min=0, max=8192, step=32, tooltip="目标高度（0=自动）。MiniMax H3 要求 32 的倍数"),
                io.Combo.Input("ref_image_size", options=["match", "max"], default="match",
                               tooltip="参考图统一缩放策略：match=按生成画布面积；max=短边 2048 高保真"),
                io.Combo.Input("crop_mode", options=["disabled", "center", "contain"], default="disabled",
                               tooltip="参考图裁剪+缩放：disabled=按宽高比缩放；center=铺满居中裁剪；contain=letterbox 填充"),
                io.String.Input("prompt_text", default="", multiline=True, optional=True),
                io.Float.Input("duration", default=0.0, min=0.0, optional=True),
                io.Combo.Input("ref_image_format", options=["jpeg", "png"], default="jpeg", optional=True),
                ...
                io.Autogrow.Input("ref_videos", optional=True,
                                  template=io.Autogrow.TemplatePrefix(
                                      input=io.Image.Input("ref_video", tooltip="参考视频帧（24fps）"),
                                      prefix="ref_video_", min=0, max=MAX_REFERENCE_VIDEOS)),
                io.Autogrow.Input("ref_video_audios", optional=True,
                                  template=io.Autogrow.TemplatePrefix(
                                      input=io.Audio.Input("ref_video_audio", tooltip="参考视频配乐"),
                                      prefix="ref_video_audio_", min=0, max=MAX_REFERENCE_VIDEOS)),
            ...
            outputs=[io.String.Output(display_name="保存路径")],
        )

    @classmethod
    def execute(cls, conditioning, seed, filename, language, canvas_mode="auto",
        ...
        ref_image_list = [img for img in (ref_images or {}).values() if img is not None]
        ref_video_list = [v for v in (ref_videos or {}).values() if v is not None]
        ref_video_audio_list = [a for a in (ref_video_audios or {}).values() if a is not None]
        ref_audio_list = [a for a in (ref_audios or {}).values() if a is not None]

        # 画布尺寸
        canvas_w, canvas_h, canvas_used_mode = resolve_canvas(canvas_mode, width, height, ref_image_list)
        ...
        ref_video_audio_tensors = [sa for a in ref_video_audio_list if (sa := _serialize_audio(a)) is not None]
        ref_audio_tensors = [sa for a in ref_audio_list if (sa := _serialize_audio(a)) is not None]
        ...
        return io.NodeOutput(path, ui={"text": [summary]})
