...
class ZouyuVAEDecodeAV:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "samples": ("LATENT", {"tooltip": "要解码的 AV latent（来自融合加载器或采样器）"}),
                "vae_image": ("VAE", {"tooltip": "视频 VAE（图像解码）"}),
                "vae_audio": ("VAE", {"tooltip": "音频 VAE（音频解码）"}),
    ...
    def _decode_audio(self, vae_audio, audio_latent, samples):
    ...
    def decode(self, samples, vae_image, vae_audio, language="中文"):
        zh = (language != "English")
        ...
        if is_nested:
            ...
            audio = self._decode_audio(vae_audio, audio_latent, samples)
        else:
            log("警告: latent 不含音频（非嵌套），音频输出为空" if zh else "Warning: latent has no audio")

        if zh:
            log(f"音视频解码完成: 图像 {list(images.shape)}, 音频 {list(audio['waveform'].shape)}")
        else:
            log(f"AV decode done: images {list(images.shape)}, audio {list(audio['waveform'].shape)}")
